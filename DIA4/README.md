...
El problema consiste en analizar un gran almacen donde los rollos de papel (`@`) estan dispuestos en una cuadricula. Cada posición de la cuadricula puede estar vacia (`.`) o contener un rollo de papel. El acceso a un rollo depende de las siguientes reglas:

- Solo se puede acceder a un rollo si en las ocho posiciones adyacentes hay menos de 4 royos de papel, si lo cumple se considera accesible.

El objetivo es contabilizar cuantos rollos son accesibles.
...
En cuanto a la técnicas y estructuras que se han implementado, han sido las siguientes: Para la lectura de archivos se ha utilizado `ifstream` y `getline()`. También se ha usado una matriz de string, que nos permitía usar el documento como una matriz. Para mirar los rollos vecinos, se ha usado desplazamientos(`df, dc`) que representaban las 8 direcciones alrededor de la celda. Esta técnica se ha usado como un grafo implícito, donde cada celda es un nodo y las aristas son las adyacencias.
